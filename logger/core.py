...
class FactoryLogging:
    ...
    def config_from_file(cfg_filename, loglevel="DEBUG"):
        """
        Logging helper based on a configuration file. This method will
        configure a valid logger object, following the next steps:

        1) If no configuration file is supplied, a basic default logger is
        provided with a StreamHandler to console.

        2) If a configuration file is supplied, the logger is configured
        by using the fileConfig method from the standard python logging
        module.
